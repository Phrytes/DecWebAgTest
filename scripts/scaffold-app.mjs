 ...
 *
 * Generates a minimal hello-world `@decwebag` app skeleton under
...
if (args.length === 0 || args.includes('--help') || args.includes('-h')) {
  process.stdout.write(`scaffold-app — generate a minimal @decwebag app skeleton
...
Arguments:
  <name>            App name (becomes the directory + the @decwebag-app/<name> package name).
...
const packageJson = {
  name:        `@decwebag-app/${name}`,
  version:     '0.0.1',
  private:     true,
  description: `Minimal @decwebag hello-world app scaffolded ${new Date().toISOString().slice(0, 10)} via scripts/scaffold-app.mjs.`,
  type:        'module',
  main:        'src/index.js',
 ...
 * ${name} — minimal hello-world @decwebag app.
 *
...
import { DataPart } from '@decwebag/core';
...
// 7. README.md.
writeFileSync(join(appDir, 'README.md'), `# @decwebag-app/${name}

Minimal hello-world \`@decwebag\` app scaffolded via \`scripts/scaffold-app.mjs\`.
...
| Storage | (none) | \`@decwebag/pseudo-pod\` for local, \`@decwebag/pod-client\` for real Solid pods |
| Fan-out | (none) | \`@decwebag/notify-envelope\` for crew-replicated writes |
| Item shape | (free-form) | \`@decwebag/item-types\` canonical taxonomy |
