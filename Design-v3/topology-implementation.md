...
extracting demo code into `@decwebag/react-native`) is in
...
**Output:** `@decwebag/pod-client` published; `SolidPodSource`
...
| # | Task | Tag | Notes |
|---|---|---|---|
| I1 | Private-server bundle | [NEW, packaging project] | Node service running `@decwebag/core` + Solid pod server (CSS) + optional ollama + admin web UI.  Yunohost / Umbrel / Cloudron manifests |
