...
```
- `expo-notifications` is a PEER dep of @decwebag/react-native, NOT a top-level
...
- Barrel deviation: only `MobilePushBridge` and `PushAdapter` are re-exported
  ...
  available via subpath import:
      import { ExpoNotificationsAdapter }
        from '@decwebag/react-native/src/transport/pushAdapters/ExpoNotificationsAdapter.js';
...
- [x] 2. Extract `verifyGroupProof(proof, expectedAdminPubKey)` standalone helper in `@decwebag/core` mirroring `GroupManager.verifyProof`'s canonical-form check; export from `packages/core/src/index.js`.
...
- `verifyGroupProof` is additively exported from `@decwebag/core` and verified by tests against real `GroupManager`-issued proofs.
...
```
2026-04-28 — E2a done.
- `verifyGroupProof` exported additively from `@decwebag/core`
...
- Q-E.2 design dictated this: NO new dependency added (relay imports
  `verifyGroupProof` via the existing `@decwebag/core` peer-dep).
