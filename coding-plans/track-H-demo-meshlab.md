...
- A **"call from outside"** terminal — a separate `a2a-cli` script
  ...
  to verify external interop, with no `@decwebag/*` import.
...
**Story:** A separate process (NOT a `@decwebag/*` consumer) — could be
...
**Story:** Alice runs `decwebag-mesh-lab broadcast "anyone have a 4mm
...
3. Runs a small in-memory relay (the existing `@decwebag/relay`).
...
constructs and orchestrates real `@decwebag/core` agents.  Every
...
```
apps/mesh-lab/
  package.json                                # name: "@decwebag-app/mesh-lab"
  src/
