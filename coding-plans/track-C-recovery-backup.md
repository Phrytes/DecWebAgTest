...
- [x] 2. Encryption: optional, with key derived from bootstrap secret via `Bootstrap.deriveResourceKey('decwebag-pod-export-v1', salt)`.  Default ON.
- [x] 3. `dataOnly` flag: omit identity (`/decwebag/`) container.
...
- [x] `dataOnly: true` skips `/decwebag/`.
...
2026-04-28 (C3 agent):
  ...
  via Bootstrap.deriveResourceKey('decwebag-pod-export-v1', salt).
