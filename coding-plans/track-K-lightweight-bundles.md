...
folio-mobile/src/ServiceContext.js
  → @decwebag-app/folio/rn/serviceFactory
      → @decwebag/pod-client (PodClient, SolidOidcAuth)
          → @decwebag/core (entire `src/index.js`)
...
exports.  None of `@decwebag/*` declare this today.
...
Then **swap `import { X } from '@decwebag/core'` for deep imports**
in folio-mobile (`@decwebag/core/src/identity/AgentIdentity.js`)
...
### Option 2 — New `@decwebag/pod-lite` package (cleanest API surface)
...
Folio + future pod-only H apps import `@decwebag/pod-lite` instead
of `@decwebag/core` + `@decwebag/pod-client`.
...
| Q-K.2 | How aggressive can the prune be without breaking @decwebag/core's existing tests? | **Run the full 1236-test core suite after each prune step.**  CI's matrix workflow makes this cheap. |
| Q-K.3 | Should `apps/folio-mobile` use `@decwebag/pod-lite` (Option 2) even if Option 1 covers most of the gap? | **TBD — depends on measurement.**  If Option 1 gets us under, say, 15 MB bundle, pod-lite isn't worth the maintenance cost. |
...
- Splitting `@decwebag/core` into multiple sub-packages (huge refactor; not worth it for the PoC).
