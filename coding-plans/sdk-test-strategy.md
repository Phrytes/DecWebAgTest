...
Scenarios are inherently cross-package: they wire `@decwebag/core` +
`@decwebag/pod-client` + `@decwebag/relay` together.  Putting them in
...
`@decwebag/*` packages as `file:` deps, identical to how Track H apps
...
```js
import { Lab } from '@decwebag/test-scenarios/harness';
