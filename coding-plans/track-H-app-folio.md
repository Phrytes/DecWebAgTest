...
No changes to `@decwebag/core`, `@decwebag/pod-client`,
`@decwebag/react-native`, or `@decwebag/relay` are required for any of
the three phases.
...
|---|---|---|
| `@decwebag/core` | `AgentIdentity`, `Bootstrap`, `IdentityPodStore`, `IdentitySync`, `Vault*` | ✅ shipped (A + B complete) |
| `@decwebag/pod-client` | `PodClient.read/list/write/append/delete`, `'conflict'` event, `TombstoneStore`, `CapabilityAuth` | ✅ shipped (A1–A7 complete) |
| `@decwebag/react-native` | `attachIdentityToAgent` (Phase C only) | ✅ shipped (B4 complete) |
| `@decwebag/relay` | not used (single-user) | n/a |
...
- For Phase C (mobile): RN background-task integration with
  ...
  belongs in `@decwebag/react-native` if/when needed.
...
#### Folio.A1 — Sync engine library (`@decwebag-app/folio-core`)
...
create:
  ...
  apps/folio/package.json                            # @decwebag-app/folio-core (or @decwebag-app/folio if we ship as one package)
...
- [x] 3. Implement `scanLocal(rootPath)` — walks the local filesystem, returns `{ path, mtime, sha256, size }[]`.  Skips `.decwebag/` metadata + dotfiles by default; configurable.
...
- A2 pre-task notes:
  ...
  * Conflict resolution UX (folio conflicts) can read `engine.stats` plus
    ...
    @decwebag-app/folio for that purpose.
  * `folio init` should write a `~/.config/folio/config.json` with
    { localRoot, podRoot, webId } and stash the BIP-39 phrase in keychain
    via the existing Vault* adapters from @decwebag/core.
  * The state-file lives under `<localRoot>/.decwebag/notes-sync-state.json`,
...
- Twists 1+2 (auto-shared with-<webid>/, time-machine) deferred per
  locked decisions.
- SyncEngine extends @decwebag/core's Emitter.  This requires
...
```
apps/folio/                                     # this app
  package.json                                  # name: "@decwebag-app/folio"
                                                # main: "src/index.js"
...
apps/folio-mobile/                              # if Phase C splits out
  package.json                                  # name: "@decwebag-app/folio-mobile"
  ...
```
...
```json
"dependencies": {
  "@decwebag/core":         "file:../../packages/core",
  "@decwebag/pod-client":   "file:../../packages/pod-client",
  "chokidar":               "^3.6.0"
},
"bin": { "folio": "src/cli.js" }
```
...
|---------|--------------------------------------------------|-----------------------------------------|
| macOS   | `~/Library/LaunchAgents/ag.decwebag.folio.plist` | `launchctl load/unload` (LaunchAgent)   |
