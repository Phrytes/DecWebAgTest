...
| Vault | `VaultNodeFs` (file-backed) | `VaultRN` using `expo-secure-store` (already exists in `@decwebag/react-native` per CLAUDE.md — confirm) |
...
| Q-C1.5 | Vault-on-mobile — use `expo-secure-store` directly, or wrap via `@decwebag/react-native`'s existing `VaultRN`? | **Wrap via `VaultRN`.**  Already-shipped surface; consistent with mesh-demo's existing identity flow. |
| Q-C1.6 | Reuse `@decwebag/pod-client`'s `CapabilityAuth` + `SolidOidcAuth` directly, or write a thin RN-aware variant? | **Reuse directly.**  The auth modules don't touch fs; they're pure crypto + http.  RN's `fetch` is compatible.  The OIDC *redirect* flow is C2's concern (see "Mobile auth flow" below) — C1 just makes sure the engine, vault, and PodClient still wire together when given an authenticated session from RN-land. |
...
4. **No `expo-secure-store` adapter.**  The plan mentioned "Vault uses
   `VaultRN` from `@decwebag/react-native`" (Q-C1.5) — that wiring is
...
apps/folio-mobile/                         — NEW workspace (Q-C1.3 lock)
  ...
  vitest.config.js                         — aliases @decwebag/* + scure +
