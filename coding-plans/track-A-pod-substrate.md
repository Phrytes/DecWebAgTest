...
modify:
  ...
  packages/core/package.json                            # depend on @decwebag/pod-client (later — when core consumers migrate)
```

**Sequence:**

- [ ] 1. Create the new package skeleton — `package.json` with `"name": "@decwebag/pod-client"`, `"type": "module"`, `vitest` dev dep, `@decwebag/core` dep.
