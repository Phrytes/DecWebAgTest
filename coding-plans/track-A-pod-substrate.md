...
### A5 — Pod-client high-level API (`@decwebag/pod-client`)
...
**Note on monorepo wiring:** the existing repo uses `file:` references (e.g. `"@decwebag/core": "file:../core"`) instead of npm workspaces.  Stick with that pattern — `packages/pod-client/package.json` declares `"@decwebag/core": "file:../core"` and runs its own `npm install`.  No root-`package.json` workspace declaration is needed.
...
- [x] 1. Create `packages/pod-client/package.json`: `"name": "@decwebag/pod-client"`, `"type": "module"`, `"main": "src/index.js"`, `"@decwebag/core": "file:../core"` dep, `vitest` dev dep, `test` script.
...
**DoD:**
- New `@decwebag/pod-client` package boots; `npm test --prefix packages/pod-client` runs Errors smoke test green.
- Root `npm test` includes pod-client and stays green across all packages.
- `import { PodClientError, AuthError, ConflictError, ... } from '@decwebag/pod-client'` resolves from a sibling package via `file:` reference.
...
- Cross-package import verified end-to-end: `import { ... } from
  '@decwebag/pod-client'` resolves Auth, Errors, mapSourceCode cleanly
...
- AsyncStorageTombstones uses dynamic import('@react-native-async-storage
  ...
  package.json change.
- FileTombstones default path is os.tmpdir()/decwebag-tombstones.json.
...
```
2026-04-29 (A7 agent):
- PodClient now extends Emitter (re-exported from @decwebag/core).
