...
| Q-Smoke.3 | Do we re-write mesh-demo screens to expose the new SDK surface (vault, pod, governance) for testing?  Or write a dedicated smoke harness? | **TBD — leaning a small smoke harness app** under `apps/sdk-smoke/`, sharing the @decwebag packages but with a stripped-down UI that surfaces the actual SDK calls (no Folio, no chat).  Faster than dressing up mesh-demo. |
