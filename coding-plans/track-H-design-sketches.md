...
```
$ decwebag notes init ~/Documents/notes
...
```
$ decwebag notes status              # how many files, last sync, conflicts
$ decwebag notes share <path>        # mint a share link
$ decwebag notes conflicts           # list + resolve conflicts interactively
$ decwebag notes pull --force        # blow away local + re-fetch from pod
```
...
/notes/
  ...
  tax-2024/                                  # private-only; ACL locked to self
    receipts.md
  .decwebag/                                 # SDK-side metadata; the user never reads this
...
(`.decwebag/history-folders.json` lists which paths to version).  Versions
...
```
$ decwebag archive search "cake recipe" --source gmail,notes
$ decwebag archive contact "mom@example.com" --since 2024-01-01
$ decwebag archive export --query "tax 2024" --bundle ~/tax-2024.zip
```
...
| Q-H7.4 | Index storage: SQLite-only (file in `~/.decwebag/archive/`) vs DuckDB (better for analytics) vs both.  Lean: SQLite + FTS5; DuckDB is overkill for a personal archive. |
...
```
$ decwebag tasks add "buy chicken" --due today --assignee me
$ decwebag tasks pick "fix kitchen tap"           # claim a "anyone" task
$ decwebag tasks done <id>
```
...
```
$ decwebag presence list                          # who's "home" right now (per shared LAN)
$ decwebag presence claim "anne-laptop"           # signed claim that I saw anne-laptop on this WiFi
$ decwebag presence why "anne-laptop"             # how confident am I + which signal sources
```
...
PER-USER POD:
  /location/
    ...
    on-lan-agents.json                     # "I saw these decwebag agent pubkeys on my LAN"
