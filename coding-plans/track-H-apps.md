...
apps/
  notes-v0/                                   # H1
    package.json                              # name: "@decwebag-app/notes-v0"
    src/
      ...
    test/
      ...
...
1. **Never** `import { X } from '../../packages/core/src/X.js'`.
   Only via the package name (`@decwebag/core`, `@decwebag/pod-client`,
   `@decwebag/react-native`).  This means the app uses **only the public
...
Until `@decwebag/core` and friends are published to npm:
