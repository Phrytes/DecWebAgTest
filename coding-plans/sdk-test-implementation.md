...
create:
  ...
  packages/integration-tests/package.json           # name: "@decwebag/integration-tests"
...
- [x] 2. Set up the `packages/integration-tests/` workspace.  `package.json` declares `file:` deps on `@decwebag/core`, `@decwebag/pod-client`, `@decwebag/relay`, plus `vitest` devDep.  `npm install` from inside the workspace.
...
- [x] No new top-level deps in `@decwebag/*` packages (test workspace is its own thing; only `vitest` as devDep + `file:` refs to existing packages).
...
Harness gaps surfaced:
  1. `vaultCacheKeyFor` (and the `VAULT_CACHE_PREFIX` constant) is exported
     by `IdentitySync.js` but NOT re-exported in `@decwebag/core`'s barrel.
     Scenarios mirror the `'identity-cache:'` prefix inline.  Tiny — could
     be added to the core barrel in a follow-up.
  2. `@decwebag/core`'s `package.json` `exports` map only exposes `"."` —
     deep imports like `@decwebag/core/src/identity/identitySerializers/manifest.js`
...
- `@decwebag/relay`'s package.json `exports` map only exposes `.`; `SqliteQueueStore` and `MultiRecipientQueue` aren't reachable via the public name.  The scenario resolves them through `node_modules/@decwebag/relay/src/...` to keep the SDK surface unchanged.  Adding sub-path exports is a low-risk follow-up.
...
- Add sub-path exports to `@decwebag/relay/package.json` so deep imports of `MultiRecipientQueue` / `SqliteQueueStore` work via the public name.
...
| 5 | Test workspace as `@decwebag/test-scenarios` (npm-named workspace, not just a tests dir) | yes | Confirm |
