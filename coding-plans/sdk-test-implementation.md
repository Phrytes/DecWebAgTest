...
create:
  ...
  test/scenarios/package.json                       # name: "@decwebag/test-scenarios"
...
- [ ] 2. Set up the `test/scenarios/` workspace.  `package.json` declares `file:` deps on `@decwebag/core`, `@decwebag/pod-client`, `@decwebag/relay`, plus `vitest` devDep.  `npm install --prefix test/scenarios`.
...
- [ ] No new top-level deps in `@decwebag/*` packages (test workspace is its own thing).
