...
import { mountLocalUi, LocalUiAuth } from '@decwebag/agent-ui';
import { CachingDataSource } from '@decwebag/local-store';
...
const { values } = parseArgs({
  options: {
    ...
    // bot bridge (TelegramBridge from @decwebag/chat-agent). The
    ...
    // stays dormant. With it, the CLI imports `@decwebag/relay`'s
...
if (values['crew-list']) {
  ...
  const { ItemStore } = await import('@decwebag/item-store');
  const { MemberMap } = await import('@decwebag/identity-resolver');
  const { DataPart }  = await import('@decwebag/core');
  ...
  for (const cfg of crewConfigs) {
    ...
    const dataSource = new (await import('@decwebag/core')).MemorySource();
...
if (values.crew) {
  ...
  if (values.push) {
    try {
      const { ExpoPushSender } = await import('@decwebag/relay');
      pushSender = new ExpoPushSender();
      console.log('  push:   Expo push sender wired');
    } catch (err) {
      console.error(`  ⚠ --push: failed to load @decwebag/relay (${err?.message ?? err})`);
    }
  }
...
if (values['telegram-token']) {
  if (!values.crew) {
    ...
    try {
      ...
      const { TelegramBridge } = await import('@decwebag/chat-agent/bridges/telegram');
