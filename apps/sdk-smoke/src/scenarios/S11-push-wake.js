...
import { MobilePushBridge } from '@decwebag/react-native';
...
export async function run({ log, sdk }) {
  ...
  try {
    const adapterMod = await import(
      '@decwebag/react-native/src/transport/pushAdapters/ExpoNotificationsAdapter.js'
    );
  ...
  if (!relay || typeof relay.registerPushToken !== 'function') {
    log('S11: FAIL — agent has no relay transport with registerPushToken (rebuild with @decwebag/core ≥ this session)');
