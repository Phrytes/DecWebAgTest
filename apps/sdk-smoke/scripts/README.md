...
1. **Install peer-deps in this app.** From repo root:
   ...
   `@decwebag/relay` (devDep, used only by these scripts).
...
- **`MobilePushBridge`** (device-side, `@decwebag/react-native`) —
  receives push, dispatches `'push'` event.
- **`ExpoNotificationsAdapter`** (peer-dep) — Expo notification listener.
- **`RelayTransport.registerPushToken`** (`@decwebag/core`, added 2026-05-04) —
  ships token to relay over the existing socket.
- **`PushTokenRegistry`** (`@decwebag/relay`) — relay's address→token map.
- **`ExpoPushSender`** (`@decwebag/relay`) — relay calls Expo's HTTP push API.
- **`server.js` `tryWakePush`** (`@decwebag/relay`) — fires push when an
