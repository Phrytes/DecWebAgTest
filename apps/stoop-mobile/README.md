...
>
> **Known direct cross-app dep:** `@decwebag-app/stoop` for the
...
- 40.2 ✅ `@decwebag/sync-engine-rn` — done in folio repair.
- 40.3 ✅ `@decwebag/oidc-session-rn` — done in folio repair.
- 40.4 ✅ Added `FileSystemAdapter` to `@decwebag/react-native`.
...
- 40.7 ✅ GPS via `expo-location` — `src/lib/geo.js` (re-exports cell helpers
  from `@decwebag-app/stoop/lib/geo`).
...
- 40.9 ✅ Native push via Expo — `src/lib/push.js` (`setupPush`,
  ...
  `ExpoNotificationsAdapter` from `@decwebag/react-native`.
...
| Package | Used for |
|---|---|
| `@decwebag/core` | `Agent`, `AgentIdentity`, `KeychainVault` (via `@decwebag/react-native`); identity bring-up via mnemonic-restore. |
| `@decwebag/react-native` | RN platform layer: polyfills, Metro preset, `KeychainVault`, `AsyncStorageAdapter`, `MdnsTransport`, `BleTransport`, `MobilePushBridge`. |
| `@decwebag/sync-engine-rn` | RN bootstrap: `createMobileBootstrap`, `createSyncEngine`, `bgRunOnce`, `defineBackgroundTask` + the BackgroundFetch helpers. |
| `@decwebag/oidc-session-rn` | RN-side Solid OIDC: `OidcSessionRN` token persistence, `useOidcSignIn` hook (at `/hook` subpath), DCR helpers. Stoop V3 pre-binds `appId: 'stoop'`. |
| `@decwebag/local-store` | `CachingDataSource`, `SyncCadence`, `createSettingsModule({appId: 'stoop', ...})`. |
| `@decwebag/identity-resolver` | `MemberMap`, `MemberMapCache`, `buildOnboardingSkills`, `matchesProfile`, `TAXONOMY`. |
| `@decwebag/item-store` | `ItemStore` for posts, chat-messages, claims, redemptions. |
| `@decwebag/chat-p2p` | `wireChat({...})` peer-to-peer chat (acceptedEnvelopeTypes: ['p2p-chat', 'stoop-chat']; emitEnvelopeType: 'stoop-chat' for back-compat with desktop Stoop). |
| `@decwebag/notifier` | `Notifier`, `UsageMetrics`, push channels, scheduled reminders. |
| `@decwebag/skill-match` | Pubsub-of-skills broadcast over the closed group + claim flow. |
| `@decwebag/pod-client` | Pod read/write/list when the user signs in with their Solid pod. |
...
`@decwebag/oidc-session-rn` substrate provides `OidcSessionRN`
...
apps/stoop-mobile/
├── README.md           ← this file
├── package.json        ← @decwebag-app/stoop-mobile
├── app.json            ← Expo config (scheme: stoop, perms)
├── babel.config.js     ← babel-preset-expo
├── metro.config.js     ← @decwebag/react-native preset + stoop-shaped pins
