...
} from '@decwebag-app/stoop';
import {
  Agent, AgentConfig, FallbackTable, InternalBus, InternalTransport,
  PeerGraph, RelayTransport, RoutingStrategy,
} from '@decwebag/core';
import { SkillMatch }                      from '@decwebag/skill-match';
// Subpath imports — pulling from `@decwebag/react-native`'s barrel
// would re-evaluate KeychainVault, whose `react-native-keychain` TS
// import vitest can't parse.  Subpath bypass.
import { MdnsTransport }       from '@decwebag/react-native/src/transport/MdnsTransport.js';
import { AsyncStorageAdapter } from '@decwebag/react-native/src/storage/AsyncStorageAdapter.js';
 ...
 *
 * @param {object} args
 * @param {import('@decwebag/core').Agent} args.meshAgent  — shared
...
export async function buildMeshAgent({ identity, label, peerGraphPrefix, relayUrl }) {
  ...
  try {
    ...
    if (!mdnsAvailable) {
      console.warn('[agentBundle] MdnsTransport native module unavailable — running internal-only. ' +
        ...
        'with @decwebag/react-native\'s autolinked MdnsPackage.');
    }
    if (mdnsAvailable) {
  ...
  // `npx @decwebag/relay` (port 8787 by default).
  ...
  // (currently 30s in @decwebag/react-native): canReach() returns
