...
describe('folio init', () => {
  it('persists config + writes a vault + creates the marker file', async () => {
    ...
    const markerRaw = await fs.readFile(join(localRoot, '.decwebag', '.folio-managed'), 'utf8');
...
describe('folio reset', () => {
  it('removes config + vault + per-folder metadata; leaves notes intact', async () => {
    ...
    await fs.access(join(localRoot, '.decwebag', '.folio-managed'));
    ...
    await expect(fs.access(join(localRoot, '.decwebag'))).rejects.toThrow();
