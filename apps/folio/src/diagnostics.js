...
import { VaultNodeFs as DefaultVaultNodeFs } from '@decwebag/core';
...
export async function runDiagnostics(reporter, deps = {}) {
  ...
  // ── 6. Marker file ────────────────────────────────────────────────────
  const markerPath = join(cfg.localRoot, '.decwebag', '.folio-managed');
  try {
  ...
  // ── 7. Sync state ─────────────────────────────────────────────────────
  const statePath = join(cfg.localRoot, '.decwebag', 'notes-sync-state.json');
