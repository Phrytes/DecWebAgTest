...
export class HouseholdAgent {
  ...
  constructor({ store, bridges, llm = null, scheduler = null } = {}) {
    ...
    // headless @decwebag/chat-agent ChatAgent: substrate handles
  ...
  async #routeMessage(msg) {
    ...
    if (parsed === null) {
      // ── Slow path: LLM via @decwebag/chat-agent (B.4) ────────
      if (this.#chatAgent) {
  ...
  /**
   * Delegate the LLM slow path to the embedded @decwebag/chat-agent
