...
`@decwebag/llm-client` v0.2.0.

---

## Substrate parser improvements (`@decwebag/llm-client` v0.2.0)
...
- Wraps `@decwebag/chat-agent`'s `ChatAgent` with the directive
...
| `@decwebag/item-store` substrate consumption | one big bucket of items with type field | one bucket per list name |
...
3. **Wire scheduler into freeform agent**: `addToList` /
   ...
   `@decwebag/notifier` scheduler consumes for digests + nudges.
...
4. **Wire pod persistence as an option**: `createPersistedListStore`
   ...
   that uses the existing `@decwebag/pod-client` (multi-device
   safety).
