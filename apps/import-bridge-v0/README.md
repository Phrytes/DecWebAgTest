...
|---|---|---|
| `@decwebag/identity-resolver` (L1h) | `PersonGraph` — cross-source Person records, auto-linked when two connectors observe the same identifier. | Cross-source identity reconciliation is reused by H4/H5/H7; the merge rules don't belong inline in the import bridge. |

## Direct SDK use

| SDK package | Primitive | Used for | Justification |
|---|---|---|---|
| `@decwebag/core` | `Emitter` | Agent extends it to surface `synced` events per imported item. | Substrate-portable emitter; substrates and apps share the same primitive — using `node:events` would break RN. |
| `@decwebag/core` | `OAuthVault`, `VaultMemory` | Per-connector OAuth credentials with auto-refresh (Google, Notion, …). | `@decwebag/oauth-vault` (L1g) was deleted 2026-05-04 as a duplicate of `core.OAuthVault`; this is the only place to compose. |
| `@decwebag/core` | `DataSource` (interface) | Target the connector writes into; one-shot, no `sync-engine` composition. | Phase 5.1 audit deviation: one-shot import has no V0 SyncEngine surface to consume; writing via `DataSource` directly is simpler than wrapping in `LiveSyncSkill`. |

The `@decwebag/sync-engine` substrate (L1a) is **not** composed: it is bidirectional-only post-Phase 5.1, and one-shot ingest doesn't fit. Apps that want webhook/polling/cursored sync should compose `core.protocol.LiveSyncSkill` directly instead of this app's pattern.
...
import { MemorySource } from '@decwebag/core';
...
```js
import { createImportAgent, GoogleDocsConnector } from '@decwebag-app/import-bridge-v0';
import { OAuthVault, VaultMemory } from '@decwebag/core';
