...
// the @decwebag/react-native metro-preset.
//
...
//
// Folio's app-specific bits (`@decwebag-app/folio` + its rn/*
...
module.exports = withDecwebagPreset({
  ...
  // Folio-specific module aliases (the preset already maps the @decwebag/*
  // SDK packages).
  extraNodeModules: {
    // Folio app package (CLI/server-side code; mobile only consumes
    // the rn/* subpaths via extraSubpathResolvers below).
    '@decwebag-app/folio': path.resolve(repoRoot, 'apps/folio'),
  ...
  // The shorter `@decwebag-app/folio` prefix in extraNodeModules can
  // silently override the longer `@decwebag-app/folio/rn/*` subpath
