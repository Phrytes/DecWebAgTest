# Changelog — @decwebag-app/tasks-mobile
...
| Package | Used for |
|---|---|
| `@decwebag/item-store` (L1b) | Per-crew task ledger with audit + DoD lifecycle + V2.7 dependency gating |
| `@decwebag/identity-resolver` (L1h) | Member webid map + `MemberMapCache` write-through; canonical user-skills profile; display + skills helpers |
| `@decwebag/skill-match` (L1e) | Pubsub-of-skills broadcast for crew-wide skill availability |
| `@decwebag/notifier` (L1f) | `PushChannel` + `PushPolicy` (humanInTheLoop, daily-cap, quiet hours) |
| `@decwebag/chat-p2p` | Appeal flow chat threads |
| `@decwebag/local-store` | `CachingDataSource` + `Settings` split |
| `@decwebag/online-cadence` | Foreground ticker + AppState bridge + bg-fetch helpers |
| `@decwebag/sync-engine-rn` (`./react`) | Skill hooks (useSkill, useAgentEvent, useSkillResult, useSettings, useMemberProfile) |
| `@decwebag/oidc-session-rn` (`./hook`) | Pod sign-in (PKCE + DCR + token storage) |
| `@decwebag/react-native` | Platform layer + 5 lifted submodules (picker, qr, mnemonic, push, i18n) + theme + components + storage + deepLinks + identity helpers |
| `@decwebag-app/tasks-v0` | V2.8 single-agent factories + role policy (platform-shell exception, locked 2026-05-08) |
