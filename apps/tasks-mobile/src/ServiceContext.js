...
import { MemberMap } from '@decwebag/identity-resolver';
import {
  bootstrapIdentity,
} from '@decwebag/react-native/identity/bootstrap';
import {
  createBundleRegistry,
} from '@decwebag/react-native/storage';
import {
  attachAppStateBridge,
  setBgRunOnce, clearBgRunOnce,
  registerBackgroundFetch, unregisterBackgroundFetch,
} from '@decwebag/online-cadence';

import {
  buildMeshAgent,
} from '@decwebag-app/tasks-v0/MeshAgent';
import {
  wireSkills,
} from '@decwebag-app/tasks-v0/wireSkills';
import {
  multiCrewResolver,
} from '@decwebag-app/tasks-v0/bundleResolver';
...
async function _defaultVaultFactory() {
  const mod = await import('@decwebag/react-native/src/identity/KeychainVault.js');
...
export function ServiceProvider({ children, boot = {} }) {
  ...
  const attachPod = useCallback(async ({ tokens, podRoot } = {}) => {
    ...
    const { SolidPodSource } = await import('@decwebag/pod-client');
