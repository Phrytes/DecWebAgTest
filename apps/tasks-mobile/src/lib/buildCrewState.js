...
import { buildStandardRolePolicy } from '@decwebag-app/tasks-v0';
import {
  buildActorAliases,
  buildActorResolverFromMembers,
} from '@decwebag-app/tasks-v0/ui/effectiveActor';
...
export async function buildCrewState({ crewConfig, localStoreBundle } = {}) {
  ...
  // for V0; when `@decwebag/agent-registry` is wired into mobile,
...
async function _memorySource() {
  const { MemorySource } = await import('@decwebag/core');
