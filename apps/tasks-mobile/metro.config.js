...
module.exports = withDecwebagPreset({
  ...
  // Tasks-mobile imports the @decwebag-app/tasks-v0 barrel for the
  ...
  // Tasks-specific module aliases (the preset already maps the @decwebag/*
  // SDK packages). Same `extraNodeModules` Trap-2 caveat as stoop-mobile:
  // overlapping prefixes go via `extraSubpathResolvers` below.
  extraNodeModules: {
    '@decwebag-app/tasks-v0':      path.resolve(repoRoot, 'apps/tasks-v0'),
    ...
    '@decwebag/online-cadence':    path.resolve(repoRoot, 'packages/online-cadence'),
  ...
  // Subpath resolvers for `@decwebag-app/tasks-v0/<sub>` imports —
  // metro's enablePackageExports stays off (per the preset), so the
  // app's `package.json#exports` field is ignored. Same pattern as
  // stoop-mobile's `@decwebag-app/stoop/{lib,locales}/*` resolvers.
  extraSubpathResolvers: [
    (moduleName, repoRoot) => {
      if (moduleName.startsWith('@decwebag-app/tasks-v0/locales/shared/')) {
        // Shared locale bundle (Phase 41.18 follow-up — see
        // Project Files/conventions/architectural-layering.md §
        // "Shared UI-glue helpers between platform shells").
        const sub = moduleName.slice('@decwebag-app/tasks-v0/locales/shared/'.length);
        return {
          filePath: path.resolve(repoRoot, 'apps/tasks-v0/locales/shared', sub + '.json'),
          type:     'sourceFile',
        };
      }
      if (moduleName.startsWith('@decwebag-app/tasks-v0/locales/')) {
        const sub = moduleName.slice('@decwebag-app/tasks-v0/locales/'.length);
        return {
          filePath: path.resolve(repoRoot, 'apps/tasks-v0/locales', sub + '.json'),
          type:     'sourceFile',
        };
      }
      if (moduleName.startsWith('@decwebag-app/tasks-v0/MeshAgent')) {
        return {
          filePath: path.resolve(repoRoot, 'apps/tasks-v0/src/MeshAgent.js'),
          type:     'sourceFile',
        };
      }
      if (moduleName.startsWith('@decwebag-app/tasks-v0/wireSkills')) {
        return {
          filePath: path.resolve(repoRoot, 'apps/tasks-v0/src/wireSkills.js'),
          type:     'sourceFile',
        };
      }
      if (moduleName.startsWith('@decwebag-app/tasks-v0/bundleResolver')) {
        return {
          filePath: path.resolve(repoRoot, 'apps/tasks-v0/src/bundleResolver.js'),
          type:     'sourceFile',
        };
      }
      if (moduleName.startsWith('@decwebag-app/tasks-v0/Crew')) {
        return {
      ...
      if (moduleName.startsWith('@decwebag-app/tasks-v0/ui/')) {
        const sub = moduleName.slice('@decwebag-app/tasks-v0/ui/'.length);
        return {
      ...
      // The preset only auto-handles `@decwebag/react-native/platform/*`
      // and `@decwebag/sync-engine/*`. Everything we lifted in Phase 41.0
      ...
      if (moduleName.startsWith('@decwebag/react-native/')) {
        const sub = moduleName.slice('@decwebag/react-native/'.length);
        if (RN_SUBS.has(sub)) {
          return {
      ...
      // @decwebag/sync-engine-rn/react — the Phase 41.0 L1 lift target.
      // Distinct from `@decwebag/sync-engine/...` (preset handles that).
      if (moduleName === '@decwebag/sync-engine-rn/react') {
        return {
      ...
      // @decwebag/identity-resolver/{display,skills} — Phase 41.0.b A1+A2 lifts.
      if (moduleName === '@decwebag/identity-resolver/display') {
        return {
          filePath: path.resolve(repoRoot, 'packages/identity-resolver/src/display.js'),
          type:     'sourceFile',
        };
      }
      if (moduleName === '@decwebag/identity-resolver/skills') {
        return {
