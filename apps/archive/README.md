...
|---|---|---|
| `@decwebag/agent-ui` (L1d) | `mountLocalUi(agent)` — exposes the archive's skills (`search`, `addSource`, `index`, …) over A2A's standard wire shape on `127.0.0.1`. | Localhost-A2A bring-up is shared across H4 / H5 / H7 (archive); the substrate owns the dispatch + agent-card path. |
| `@decwebag/pod-search` (L1i) | SQLite FTS5 indexer + faceted search. | The pod-walker + index schema are reused by future search UIs; this app drives the substrate's V1 contract (compose `pod-client.PodClient.list/read` + tombstone eviction via `'delete-local'` event). |

## Direct SDK use

| SDK package | Primitive | Used for | Justification |
|---|---|---|---|
| `@decwebag/core` | `Agent`, `AgentIdentity`, `VaultMemory`, `InternalBus`, `InternalTransport` | Constructing the archive's local agent that `mountLocalUi` exposes over A2A. | No substrate wraps "construct an agent" — that's the SDK foundation. The CLI runs the agent in-process, no relay needed. |
| `@decwebag/core` | `defineSkill` | Registering the archive's skill handlers (search / addSource / index / status / show). | Skill-registry primitive is SDK-foundational; every consumer's skill set differs, so no substrate wraps the registry call site. |
| `@decwebag/pod-client` | (Future, real-pod path) `PodClient` | The walker today goes through `FsBackedMockPodClient`; real Solid OIDC plugs in `pod-client.PodClient`. | Same justification as folio: `PodClient` is the canonical pod read/write primitive; substrates compose it, apps construct it. Tracked in the "What v0 deliberately doesn't ship" list. |
...
apps/archive/
├── README.md             ← this file
├── package.json          ← @decwebag-app/archive
