...
**The agent SDK is four packages — `@decwebag/core`, `@decwebag/relay`,
`@decwebag/pod-client`, `@decwebag/react-native`** (described below).
...
- **Local-only mode is the floor; pod is portability.** Every app
  ...
  `MergeContracts` + `@decwebag/relay`'s `group-publish` for P2P
...
- **Network identity rotation by default.** `Agent.rotateIdentity()`
  in `@decwebag/core` (Group FF) rotates the agent's Ed25519 keypair
...
- **`@decwebag/core`** — pure JS, runs in browser, Node, and React Native.
...
- **`@decwebag/relay`** — Node-only WebSocket relay server. Rendezvous +
  proxy fallback + multi-recipient fan-out + group auth + push wake (E2c).
- **`@decwebag/pod-client`** — high-level Solid pod client: read, write,
  list, conflict resolution, tombstone tracking.
- **`@decwebag/react-native`** — RN platform layer: BLE, mDNS,
...
|---|---|---|
| **mDNS / TCP** | Same Wi-Fi LAN | `MdnsTransport` (`@decwebag/react-native`) |
| **BLE** | Out of Wi-Fi range, in Bluetooth range (≈10 m) | `BleTransport` (`@decwebag/react-native`) |
| **WebRTC DataChannel** | Across networks, after rendezvous signalling completes | `RendezvousTransport` (`@decwebag/core`) |
...
`@decwebag/relay` is the SDK's WebSocket relay implementation.  Two modes:
...
`NknTransport` (`@decwebag/core`) connects to the
...
```js
import { RelayTransport } from '@decwebag/core';
