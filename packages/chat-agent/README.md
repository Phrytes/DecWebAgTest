...
```js
import { ChatAgent, InMemoryBridge } from '@decwebag/chat-agent';
import { LlmClient } from '@decwebag/llm-client';
import { ollamaProvider } from '@decwebag/llm-client/providers/ollama';
...
- `@decwebag/llm-client` — the LLM provider abstraction this consumes.
- `@decwebag/notifier` — paired substrate; subscribes to `tool-call` events.
