...
Pure-JS core of the @decwebag SDK.  Runs in browser, Node, and React
...
- **A2A bridge** — JSON-RPC over HTTPS interop with non-`@decwebag`
  agent frameworks.
...
| `RelayTransport` | Centralized relay | WebSocket to `@decwebag/relay`.  Two server-side modes: rendezvous (signalling) + proxy fallback. |
...
`@decwebag/react-native` because they need native modules.
...
Phone factory (`createMeshAgent`) lives in `@decwebag/react-native`.
