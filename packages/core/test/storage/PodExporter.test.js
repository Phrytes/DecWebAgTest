 ...
 *   - dataOnly: true skips /decwebag/ identity container
...
function fixturePod() {
  return makeMockPod({
    initial: {
      ...
      [POD_ROOT + 'decwebag/identity.json']: { content: '{"v":1}', contentType: 'application/json' },
      [POD_ROOT + 'decwebag/devices/dev1.enc']: { content: new Uint8Array([0xab, 0xcd]), contentType: 'application/octet-stream' },
    },
  });
}
...
describe('PodExporter (unencrypted)', () => {
  ...
  it('dataOnly: true skips entries under /decwebag/', async () => {
    const src = fixturePod();
    const exporter = new PodExporter({ podClient: src, podRoot: POD_ROOT });
    ...
    expect(dst._store.has(POD_ROOT + 'decwebag/identity.json')).toBe(false);
    expect(dst._store.has(POD_ROOT + 'decwebag/devices/dev1.enc')).toBe(false);
