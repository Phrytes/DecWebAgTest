 ...
 *   • walks the `/decwebag/` container and computes the deterministic
...
export class IdentityPodStore {
  ...
  /** @type {string} */    #podRoot;        // e.g. 'https://alice.example/decwebag/'
  /** @type {string} */    #rootPubKey;     // base64url Ed25519 pubkey of #identity.

  /**
   * @param {object} opts
   * @param {object}        opts.podClient   `@decwebag/pod-client` PodClient.
   ...
   *                                          `<podRoot>/decwebag/`.  If `podRoot`
   *                                          already ends in `/decwebag/`, it
   *                                          is used as-is.
   */
  constructor({ podClient, bootstrap, identity, podRoot } = {}) {
    ...
    // Normalize: ensure podRoot ends with '/decwebag/'.
    let root = podRoot;
    if (!root.endsWith('/')) root += '/';
    if (!root.endsWith('/decwebag/')) root += 'decwebag/';
    this.#podRoot = root;
  }

  /** Identity container root URI (always ending in `/decwebag/`). */
