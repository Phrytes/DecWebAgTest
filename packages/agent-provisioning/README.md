# @decwebag/agent-provisioning

> **Layer: facade.** One-call agent bring-up. Composes
> [`@decwebag/core`](../core/) + the standardisation substrates
> (`@decwebag/vault`, `@decwebag/oidc-session`,
> `@decwebag/webid-discovery`, plus the forthcoming
> `@decwebag/pseudo-pod` and `@decwebag/agent-registry`) into a
...
```js
import { provisionAgent } from '@decwebag/agent-provisioning';

const { agent, identity, vault, mnemonic, oidc, webid } =
  await provisionAgent({
    // required
    transport,                         // any @decwebag/core Transport
...
```
                  Apps (Tasks, Stoop, Folio)
                          │
                          ▼
        @decwebag/agent-provisioning  ← this package (facade)
                          │
        ┌───────┬─────────┼─────────┬───────────────────┐
        ▼       ▼         ▼         ▼                   ▼
  @decwebag/  @decwebag/  @decwebag/  @decwebag/   @decwebag/
...
- pod-having bring-up with mocked OIDC (Inrupt session injected
  via `_setSessionFactory` from `@decwebag/oidc-session`)
- WebIdCache wiring with the pseudo-pod's read

## Limitations / future work

- The not-yet-built substrates (`@decwebag/pseudo-pod`,
  `@decwebag/agent-registry`, `@decwebag/pod-onboarding`) are
...
- First-run pod provisioning (creating the
  ...
  pointer predicates) waits for the `@decwebag/pod-onboarding`
...
- [`@decwebag/core`](../core/) — Agent + transport + skill
  registry + identity primitives.
- [`@decwebag/vault`](../vault/) — Vault family.
- [`@decwebag/oidc-session`](../oidc-session/) — Solid OIDC for
  Node.
- [`@decwebag/oidc-session-rn`](../oidc-session-rn/) — Solid
  OIDC for React Native (the RN provisioning analogue lives
  in `@decwebag/sync-engine-rn`'s `createMobileBootstrap`).
- [`@decwebag/webid-discovery`](../webid-discovery/) — WebID
