...
The substrate has **no runtime dependency on `@decwebag/core`** or
on `@decwebag/pod-client`. Callers wire in their own `fetch`
...
- [`@decwebag/oidc-session`](../oidc-session/) — typically supplies
  `fetch` for authenticated profile reads.
- [`@decwebag/pseudo-pod`](../pseudo-pod/) (forthcoming, P1) —
