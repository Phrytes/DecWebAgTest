 ...
 * in `@decwebag/oidc-session`: try JSON-LD first, then Turtle regex.
 ...
 *   <webid> <https://decwebag.org/ns#storage-mapping-uri> <uri> .
 *
 * Recognised predicate forms (JSON-LD):
 *   "storage-mapping-uri": "<uri>"
 *   "https://decwebag.org/ns#storage-mapping-uri": [{"@id": "<uri>"}]
 *
...
function matchTurtlePredicate(body, fullIri, localName, _webidUri) {
  ...
  const reShort  = new RegExp(`(?:dec|decwebag):${localEsc}\\s*<([^>]+)>`, 'i');
