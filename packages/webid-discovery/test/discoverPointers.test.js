...
describe('parseWebIdPointers — JSON-LD', () => {
  it('extracts pointers using full-IRI keys', () => {
    const body = JSON.stringify({
      '@id': WEBID,
      'https://decwebag.org/ns#storage-mapping-uri': 'https://alice.pod/private/storage-mapping',
      'https://decwebag.org/ns#agent-registry-uri':  'https://alice.pod/private/agent-registry',
    });
  ...
  it('extracts URIs from @id-wrapped JSON-LD objects', () => {
    const body = JSON.stringify({
      '@id': WEBID,
      'https://decwebag.org/ns#agent-registry-uri': [{ '@id': 'https://alice.pod/private/agent-registry' }],
    });
    expect(parseWebIdPointers(body, WEBID)).toEqual({
      agentRegistryUri: 'https://alice.pod/private/agent-registry',
    });
...
describe('parseWebIdPointers — Turtle', () => {
  it('extracts pointers using full-IRI predicates', () => {
    const body = `
      <${WEBID}> <https://decwebag.org/ns#storage-mapping-uri> <https://alice.pod/private/storage-mapping> .
      <${WEBID}> <https://decwebag.org/ns#agent-registry-uri>  <https://alice.pod/private/agent-registry> .
    `;
    expect(parseWebIdPointers(body, WEBID)).toEqual({
      storageMappingUri: 'https://alice.pod/private/storage-mapping',
      agentRegistryUri:  'https://alice.pod/private/agent-registry',
  ...
  it('extracts pointers using dec: prefix predicates', () => {
    const body = `
      @prefix dec: <https://decwebag.org/ns#> .
...
describe('discoverPointers — fetch integration', () => {
  it('fetches the WebID profile and returns parsed pointers + raw body', async () => {
    const body = `<${WEBID}> <https://decwebag.org/ns#storage-mapping-uri> <https://alice.pod/private/storage-mapping> .`;
