...
function turtleProfile({ storageMapping, agentRegistry, auditLog } = {}) {
  ...
  if (storageMapping) lines.push(`<${WEBID}> <https://decwebag.org/ns#storage-mapping-uri> <${storageMapping}> .`);
  if (agentRegistry)  lines.push(`<${WEBID}> <https://decwebag.org/ns#agent-registry-uri>  <${agentRegistry}> .`);
  if (auditLog)       lines.push(`<${WEBID}> <https://decwebag.org/ns#audit-log-uri>       <${auditLog}> .`);
  return lines.join('\n');
}
