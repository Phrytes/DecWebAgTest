...
The `@decwebag` SDK layered as **apps → substrates → core**.
...
| Package | Role |
|---|---|
| `@decwebag/core` | Agent, Transport, identity, security, skills, permissions interfaces. Never imports substrates. |

### Storage + Solid

| Package | Role | Phase |
|---|---|---|
| `@decwebag/pseudo-pod` | Solid-shaped local store. V0 standalone + replication-ring; V1 cache mode + write-through queue. | 52.2 + 52.8 |
| `@decwebag/pod-routing` | Storage-function → URI mapping. Per-write reachability gate for graceful degradation. | 52.3 |
| `@decwebag/pod-client` | High-level Solid pod read/write with capability + OIDC auth. V2 hook: optional `pseudoPod` injection routes `pseudo-pod://` URIs locally. | 52.6.3 |
| `@decwebag/pod-onboarding` | Provisioning orchestration. `provisionDefault`, `restoreFromMnemonic`, `signOut`. Provider-agnostic via injected `podProvisioner`. | 52.5 |
| `@decwebag/item-store` | Item-shaped state with role-policy gates. V2 hooks: lifted DAG helpers + `treeOf` cross-pod embed walk. | 52.6.1 + 52.6.2 |
| `@decwebag/item-types` | Cross-app canonical type taxonomy (`task`, `note`, `chat-message`, `offer`, `request`, `claim`, `contact`, `calendar-event`, `announcement`, `reveal-request`, `neighbourhood-job`). JSON-Schema validator + `validateCanonical` adapter for item-store-shaped items. Vocabulary refresh 2026-05-12 — `offer` / `request` / `claim` replace the earlier `supply-offer` / `demand-offer` / `lend-request` (legacy names persist as aliases). | 52.1 + 52.7 |
| `@decwebag/sync-engine` | Pre-V2 pod-sync. Absorbed by pseudo-pod cache mode in V1 (Phase 52.8 acceptance work; legacy callers still work). | (pre-V2) |
| `@decwebag/local-store` | In-memory/disk store used by tasks/household. Pre-V2; pseudo-pod V0 covers the same surface. | (pre-V2) |
| `@decwebag/pod-search` | Pod-side search index. Unchanged in V2. | (pre-V2) |

### Identity + agents

| Package | Role | Phase |
|---|---|---|
| `@decwebag/vault` | Pluggable secret store (memory, OS keychain, etc.). | (pre-V2) |
| `@decwebag/oidc-session` / `-rn` | Solid OIDC session management. Provides the authenticated `fetch` to `pod-client`. | (pre-V2) |
| `@decwebag/webid-discovery` | WebID profile pointer walk. `discoverPointers` reads `dec:storage-mapping-uri` etc. off the user's profile. | (pre-V2; ns updated for V2) |
| `@decwebag/agent-registry` | Canonical agent list per user. Etag-based optimistic concurrency. Implements core's `ActorResolver` via `makeActorResolver`. V0 store is on the pseudo-pod (pod-side mirroring is cache-mode V1+ work). | 52.10 |
| `@decwebag/identity-resolver` | WebID ↔ display-name resolution + member maps + reveals. V2 hook: `createAgentRegistryMemberMap(registry)` adapts an agent-registry into the MemberMap surface. | 52.11 |

### Messaging + delivery

| Package | Role | Phase |
|---|---|---|
| `@decwebag/relay` | NKN relay infrastructure. Opaque transport. | unchanged |
| `@decwebag/notify-envelope` | Persistent-content writes mediator. Per-write mode picker (envelope-only vs full-payload); pending-pod-upload queue for graceful degradation. Receiver-side `writeFromPeer` integration. | 52.4 |
| `@decwebag/notifier` | Scheduler for recurring digests + one-shot nudges. V2 hook: `createEnvelopeBridge` for time-shifted envelope delivery. | 52.9 |
| `@decwebag/chat-agent` | Chat-with-LLM helpers + bridge interface. | unchanged |
| `@decwebag/chat-p2p` | P2P chat threads. Ephemeral content stays on relay-fan-out. | unchanged |
| `@decwebag/skill-match` | Pubsub-of-skills broadcast. Routes via `notifier`. | unchanged surface |
| `@decwebag/online-cadence` | Heartbeat / online-status. | unchanged |

### Direction-only (Hub V2 territory)

| Package | Role | Phase |
|---|---|---|
| `@decwebag/interface-registry` | Per-type renderer registry. `register({type, bundleId, renderer: {compact, full}})` + lookup + permission-denied fallback. | 52.12 (direction-only) |
| `@decwebag/protocol` | State-machine substrate over items. `defineProtocol` + `createProtocolOrchestrator`. Canonical first consumer: `PROPOSE_SUBTASK`. | 52.13 (direction-only) |

### Facade + RN platform

| Package | Role | Phase |
|---|---|---|
| `@decwebag/agent-provisioning` | One-call agent bring-up facade. Composes core + vault + oidc-session + webid-discovery; optional pseudo-pod + agent-registry slots. | 50.5.b |
| `@decwebag/react-native` | RN platform layer + adapters. V2 additions: `pseudo-pod-adapter` (FsBackend + AsBackend + size-routing composite + persistent dirty-set), `hub-discovery`, `hub-binding` (AIDL surface + JS-side wrappers, native modules direction-only). | 51.1–51.10 |

### Cross-cutting

| Package | Role |
|---|---|
| `@decwebag/llm-client` | LLM API client. Unchanged. |
| `@decwebag/agent-ui` | Desktop UI primitives (`mountLocalUi`, status widgets). Unchanged. |
| `@decwebag/integration-tests` | Cross-component scenario tests including the substrates-v2 end-to-end pipeline. |
...
```js
import { provisionDefault }          from '@decwebag/pod-onboarding';
import { createPseudoPod, createMemoryBackend } from '@decwebag/pseudo-pod';
import { createAgentRegistry, makeActorResolver } from '@decwebag/agent-registry';
import { createPodRouting }          from '@decwebag/pod-routing';
import { PolicyEngine }              from '@decwebag/core';
...
import { createBackend } from '@decwebag/react-native/pseudo-pod-adapter';
...
import { createHubDiscovery } from '@decwebag/react-native/hub-discovery';
import { bind }               from '@decwebag/react-native/hub-binding';
...
|---|---|
| **50.x** (core) | `@decwebag/core` ActorResolver / HubDelegateTransport / opaque slots / `agent-provisioning` facade |
| **51.1–51.4** | `@decwebag/react-native/pseudo-pod-adapter` |
| **51.5** | persistent dirty-set in `pseudo-pod-adapter` |
| **51.6–51.10** | `@decwebag/react-native/hub-discovery` + `/hub-binding` + AIDL surface |
| **52.1** | `@decwebag/item-types` (taxonomy + validator) |
| **52.2** | `@decwebag/pseudo-pod` V0 |
| **52.3** | `@decwebag/pod-routing` |
| **52.4** | `@decwebag/notify-envelope` |
| **52.5** | `@decwebag/pod-onboarding` |
| **52.6** | `@decwebag/item-store` + `@decwebag/pod-client` extensions |
| **52.7** | per-app `item-types` adoption (Tasks ✓, Stoop ✓, Folio N/A) |
| **52.8** | `@decwebag/pseudo-pod` V1 cache mode |
| **52.9** | `@decwebag/notifier` envelope bridge |
| **52.10** | `@decwebag/agent-registry` |
| **52.11** | `@decwebag/identity-resolver` agent-registry adapter; Tasks `actorResolver` opt |
| **52.12** | `@decwebag/interface-registry` (direction-only) |
| **52.13** | `@decwebag/protocol` (direction-only) |
...
`@decwebag/agent-provisioning` facade wire substrates into core via
...
`@decwebag/integration-tests/test/scenarios/substrates-v2/substrate-pipeline.scenario.test.js`
