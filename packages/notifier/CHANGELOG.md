...
- **`Channel` is now an alias for `MessagingBridge`** from
  `@decwebag/chat-agent`. Channel implementations must expose
...
- **`RecordingChannel` deleted.** Tests use `InMemoryBridge` from
  `@decwebag/chat-agent` (same shape: outbox + clearOutbox).
...
- **`@decwebag/core` becomes the only runtime dep.** `@decwebag/chat-agent`
...
- **`core.genId` is now barrel-exported** from `@decwebag/core` (was
  only available via the internal `Envelope.js` subpath). Substrates +
  apps can now `import { genId } from '@decwebag/core'` directly.
