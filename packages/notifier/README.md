...
```js
import { Notifier, PushChannel, InMemoryScheduleStore } from '@decwebag/notifier';
// Channels for chat are L1c MessagingBridge instances — pass directly.
import { InMemoryBridge } from '@decwebag/chat-agent';
// Push channel composes any relay.PushSender concrete.
import { ExpoPushSender } from '@decwebag/relay';
...
A channel **IS** an `@decwebag/chat-agent` `MessagingBridge` — the
...
- `@decwebag/chat-agent` — supplies the `MessagingBridge` typedef + `InMemoryBridge` test fake. Apps pass any bridge directly as a notifier channel.
- `@decwebag/relay` — supplies `PushSender` (abstract) + `ExpoPushSender` (concrete) for `PushChannel`.
- `@decwebag/item-store` — emits `item-added` etc. that notifier subscribes to.
