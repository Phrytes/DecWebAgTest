# `@decwebag/sync-engine-rn`

> **Layer:** SDK foundation (RN sibling of `@decwebag/sync-engine`).
> **Cross-platform sibling:** [`@decwebag/sync-engine`](../sync-engine/).
...
React Native bootstrap helpers for the decwebag agent SDK on mobile.
Cross-platform sync logic stays in `@decwebag/sync-engine`; this
...
```js
import { createReactBindings } from '@decwebag/sync-engine-rn/react';
...
{
  "dependencies": {
    "@decwebag/sync-engine-rn": "file:../../packages/sync-engine-rn",
    "@decwebag/oidc-session-rn": "file:../../packages/oidc-session-rn",
...
```js
import { defaultPodFactory } from '@decwebag/sync-engine-rn';
...
```js
import { createMobileBootstrap } from '@decwebag/sync-engine-rn';
...
## Boundary with `@decwebag/sync-engine` (the cross-platform substrate)

| Concern | Lives in |
|---|---|
| `SyncEngine`, `PathMap`, `scanLocal`, `scanPod`, `diff`, version helpers | `@decwebag/sync-engine` |
...
`@decwebag/sync-engine`, RN in `@decwebag/react-native` |
...
`@decwebag/pod-client`'s `SolidOidcAuth`. When the Inrupt-cleanup
...
The `defaultPodFactory` test stubs `@decwebag/pod-client` via
