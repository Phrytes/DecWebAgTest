...
vi.mock('@decwebag/sync-engine/adapters/fsRN', () => ({
  createFsRN: vi.fn((args) => ({ kind: 'fs', args })),
}));
vi.mock('@decwebag/sync-engine/adapters/hashRN', () => ({
  createHashRN: vi.fn((args) => ({ kind: 'hash', args })),
}));
vi.mock('@decwebag/sync-engine/adapters/watcherRN', () => ({
  createWatcherRN: vi.fn((args) => ({ kind: 'watcher', args })),
  DEFAULT_POLL_INTERVAL_MS: 10_000,
}));
vi.mock('@decwebag/sync-engine/SyncEngine', () => {
