# Changelog — @decwebag/react-native
...
- **hub-binding** sub-path (Phases 51.7 – 51.9). Promise-based
  ...
  - `negotiateVersion({clientVersions, hubVersions})` exposed as a
    pure helper.
  - AIDL interface files committed at `android/aidl/com/decwebag/hub/`:
...
- **pseudo-pod-adapter** sub-path (Phases 51.1 – 51.4). RN-side
  `StorageBackend` implementations for `@decwebag/pseudo-pod`:
  ...
  Importable as `@decwebag/react-native/pseudo-pod-adapter`.
...
- **Platform layer** (cross-cutting RN plumbing).  This package
  ...
  also serve as the RN platform layer that every `@decwebag` app
...
- **`metro-preset.cjs`** — exported reusable Metro preset.  Apps
  ...
  module.exports = withDecwebagPreset({ projectRoot: __dirname, repoRoot: '...' });
  ```
  ...
  (Folio's `@decwebag-app/folio/rn/*` subpath, app-specific
...
- The existing `@decwebag/react-native` barrel
...
- New apps starting on RN should consume the preset directly:
  - `apps/<my-app>/metro.config.js` → `withDecwebagPreset(...)`
  - `apps/<my-app>/index.js` → `import '@decwebag/react-native/platform/polyfills';` first.
