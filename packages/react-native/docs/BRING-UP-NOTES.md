# BRING-UP-NOTES — getting `@decwebag` apps to bundle on React Native

**Audience:** anyone working on a `@decwebag` RN app (Folio mobile, future
...
> `withDecwebagPreset`, you are going to rediscover most of the traps
> below the hard way.
...
### Trap 2 — `Unable to resolve "@decwebag-app/folio/rn/serviceFactory"`
...
module.exports = withDecwebagPreset({
  ...
  extraSubpathResolvers: [
    (moduleName, repoRoot) => {
      if (moduleName.startsWith('@decwebag-app/folio/rn/')) {
        const sub = moduleName.slice('@decwebag-app/folio/rn/'.length);
        return {
...
|--------------------------------------|---------------------------------------------|-----------------------------------------|
| `@decwebag-app/stoop/lib/geo`        | `apps/stoop/src/lib/geo.js`                 | `apps/stoop/lib/geo` (404)              |
| `@decwebag-app/stoop/locales/en`     | `apps/stoop/locales/en.json`                | `apps/stoop/locales/en` (404 — no .js)  |

Adding `@decwebag-app/stoop/lib/geo` to `extraNodeModules` looked
correct but was silently shadowed by the parent-prefix
`@decwebag-app/stoop` entry. Vitest didn't catch it (Vite's alias
...
**Symptom:** during `await import('@decwebag-app/folio/rn/serviceFactory')`
...
[expo-notifications] Default FirebaseApp is not initialized in
  ag.decwebag.sdksmoke. Make sure to call FirebaseApp.initializeApp(Context)
  first.
```
...
package id (`ag.decwebag.sdksmoke`) so we must provide our own.
...
2. **Add an Android app** to the project. Package name MUST match
   `app.json` `expo.android.package` exactly (`ag.decwebag.sdksmoke`).
...
### Trap 19 — `Unable to resolve "@decwebag/relay" / "web-push"` from Stoop barrel

**Symptom (Stoop V3 mobile Phase 40.23, 2026-05-08):**

```
Android Bundling failed 8041ms index.js (2063 modules)
Unable to resolve "@decwebag/relay" from "../stoop/src/lib/WebPushSender.js"
```
...
`@decwebag/relay` → `web-push`.  Both are Node-only server packages.
...
Mobile apps that compose `@decwebag-app/stoop` (the platform-shell
...
It bundles for two reasons: (a) it's a transitive dep of `@decwebag/core`,
...
- Mobile would speak REST/WS to that agent instead of bundling
  `@decwebag/core`'s server-shape paths.
