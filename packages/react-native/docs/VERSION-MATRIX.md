# VERSION-MATRIX — pinned versions for the `@decwebag` RN platform layer

The matrix below is what Folio's mobile bring-up landed on
2026-04-30.  Apps consuming `@decwebag/react-native` should pin to
...
4. **A new `@decwebag` substrate needs an API only available in a
