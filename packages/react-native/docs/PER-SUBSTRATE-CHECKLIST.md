...
3. **Identify the substrate's RN-specific surface.**  What needs to
   ...
   - Transport adapters (already in `@decwebag/react-native/adapters`)
...
service-factory helper from `@decwebag/react-native/platform`:

```js
import { selectPlatform } from '@decwebag/react-native/platform/service-factory';
...
Every RN app must import polyfills before any other `@decwebag`
...
import '@decwebag/react-native/platform/polyfills';   // FIRST
...
1. **Don't add it to your substrate.**  Add it to
   `@decwebag/react-native` (this package).
