 ...
 *      `PodClient` (we deliberately do NOT import `@decwebag/pod-client`
...
import { Bootstrap, IdentityPodStore } from '@decwebag/core';
 ...
 * @param {object} opts.pod.podClient                      caller-supplied PodClient (from `@decwebag/pod-client`)
 * @param {string} opts.pod.podRoot                        pod root URI; identity container will be at `<podRoot>/decwebag/`
 * @param {number} [opts.pod.intervalMs=300_000]           IdentitySync polling interval (5 min default per Q-B.4)
 * @param {Function} [opts.pod._identitySyncCtor]          (test-only) inject IdentitySync constructor; default: lazy `import('@decwebag/core').IdentitySync`
...
export async function attachIdentityToAgent({ vault, identity, pod } = {}) {
  ...
  if (!IdentitySyncCtor) {
    const core = await import('@decwebag/core');
    IdentitySyncCtor = core.IdentitySync;
    if (!IdentitySyncCtor) {
      throw new Error(
        'attachIdentityToAgent: IdentitySync is not exported from @decwebag/core. ' +
  ...
  // targets via @decwebag/core, etc.) construct successfully.  We swallow
