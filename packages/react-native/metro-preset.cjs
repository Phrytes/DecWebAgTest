// metro-preset — reusable Metro configuration for `@decwebag` apps
// running on React Native.
//
// This preset captures the cross-cutting Metro setup that every
// `@decwebag` RN app needs: NODE_BUILTINS shimming, `node:`-prefix
...
//   const { withDecwebagPreset } =
//     require('@decwebag/react-native/metro-preset');
//
...
// preset inside `apps/<x>/node_modules/@decwebag/react-native/` as a
...
/**
 * Build the Metro config for a `@decwebag` RN app.
 *
 ...
 */
function withDecwebagPreset(options) {
  const {
    projectRoot,
    repoRoot,
    watchFolders          = [],
  ...
  if (!projectRoot) {
    throw new Error('[withDecwebagPreset] projectRoot is required');
  }
  if (!repoRoot) {
    throw new Error('[withDecwebagPreset] repoRoot is required');
  }
  ...
  config.resolver = {
    ...
    extraNodeModules: {
      ...
      '@decwebag/pod-client':   path.resolve(repoRoot, 'packages/pod-client'),
      '@decwebag/react-native': path.resolve(repoRoot, 'packages/react-native'),
      '@decwebag/sync-engine':  path.resolve(repoRoot, 'packages/sync-engine'),
    ...
    resolveRequest: (context, moduleName, platform) => {
      ...
      // 5. `@decwebag/react-native/platform/*` — substrate's platform
      ...
      if (moduleName.startsWith('@decwebag/react-native/platform/')) {
        const sub = moduleName.slice('@decwebag/react-native/platform/'.length);
      ...
      // 5b. `@decwebag/sync-engine/*` — exports field is ignored
      ...
      if (moduleName.startsWith('@decwebag/sync-engine/')) {
        const sub = moduleName.slice('@decwebag/sync-engine/'.length);
        return {
...
module.exports = { withDecwebagPreset, NODE_BUILTINS };
