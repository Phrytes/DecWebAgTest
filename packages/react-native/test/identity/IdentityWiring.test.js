 ...
 * When B3 lands its IdentitySync export in `@decwebag/core`, the production
 * code path (no `_identitySyncCtor`) takes over automatically.
 */
import { describe, it, expect, beforeEach, vi } from 'vitest';
import { AgentIdentity, VaultMemory, Bootstrap } from '@decwebag/core';
...
describe('attachIdentityToAgent (B4)', () => {
  ...
  it('happy path: constructs Bootstrap + IdentityPodStore + IdentitySync; sync is running', async () => {
    ...
    expect(wiring.podStore.root).toBe('https://alice.example/decwebag/');
