...
describe('buildInitialAgentRegistry', () => {
  it('seeds with this agent as the sole entry', () => {
    ...
    // Seed shape matches @decwebag/agent-registry expectations
...
describe('pointerPredicates', () => {
  it('returns the predicate IRIs for each pointer', () => {
    ...
    expect(preds.storageMappingUri).toMatch(/^https:\/\/decwebag\.org\/ns#/);
    expect(preds.agentRegistryUri).toMatch(/^https:\/\/decwebag\.org\/ns#/);
    expect(preds.auditLogUri).toMatch(/^https:\/\/decwebag\.org\/ns#/);
  });
});
