...
describe('provisionDefault — happy path', () => {
  it('orchestrates the full provision flow', async () => {
    ...
    expect(patchCall.args.predicates.storageMappingUri).toMatch(/decwebag\.org/);
  });

  it('accepts a pre-built identity (no mnemonic)', async () => {
    const pseudoPod = mkPseudoPod('d');
    const prov = fakeProvisioner();
    const { AgentIdentity } = await import('@decwebag/core');
    const { VaultMemory }   = await import('@decwebag/vault');
