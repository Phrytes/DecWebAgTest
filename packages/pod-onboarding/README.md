...
```js
import { createPodOnboarding } from '@decwebag/pod-onboarding';
import { generateMnemonic }    from '@decwebag/core';
...
`@decwebag/pod-client` extensions in Phase 52.6.
...
```js
import { defaultAcpTemplates, MODES } from '@decwebag/pod-onboarding';
...
- **Validate ACP templates against the target server.** The templates
  ...
  problem.
- **Manage OIDC session refresh.** That's `@decwebag/oidc-session`'s
