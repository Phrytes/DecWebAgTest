# `@decwebag/oidc-session-rn`

> **Layer:** SDK foundation (RN-specific).
> **Cross-platform sibling:** desktop OIDC lives in
> [`@decwebag/oidc-session`](../oidc-session/) via `createSolidAuthNode()`.
...
- `OidcSessionRN` — token persistence to a `SecureStore`-shaped store
  ...
  `@decwebag/pod-client`'s `SolidOidcAuth` interface.
...
- `KNOWN_ISSUERS`, `DEFAULT_ISSUER_ID`, `resolveIssuer()` — curated
  multi-issuer list mirrored from `@decwebag/oidc-session`. Lets RN
  apps accept Inrupt + community + self-hosted Solid servers without
  hardcoding URLs.
- `<IssuerPicker>` at the `/picker` subpath
  (`@decwebag/oidc-session-rn/picker`) — drop-in component for
...
in this package + `@decwebag/oidc-session` (Node sibling) gains
...
capabilities}` to `@decwebag/pod-client`; this substrate is
unaffected.
...
```js
import { OidcSessionRN } from '@decwebag/oidc-session-rn';
...
```js
import { useOidcSignIn, OidcSessionRN } from '@decwebag/oidc-session-rn';
