...
`@decwebag/core`. Apps and other substrates depend on it directly.
...
- the **canonical IRI** (`https://decwebag.org/ns#Task`);
...
```js
import { validate, list, metadata } from '@decwebag/item-types';
...
metadata('task');    // → { name: 'task', iri: 'https://decwebag.org/ns#Task' }
```
...
```js
import { createRegistry, registerCanonicalTypes } from '@decwebag/item-types';
...
(`https://decwebag.org/ns#<TypeName>`). The substrate intentionally does
