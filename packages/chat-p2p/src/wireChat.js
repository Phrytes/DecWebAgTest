 ...
 *
 * @param {object} args
 * @param {import('@decwebag/core').Agent} args.agent
 * @param {import('@decwebag/item-store').ItemStore} args.itemStore
 * @param {import('@decwebag/identity-resolver').MemberMap} args.members
  ...
  async function handleIncoming({ from: fromPubKey, parts }) {
    ...
    if (data.subtype === 'broadcast-post') {
      ...
      try {
        ...
        // @decwebag/identity-resolver since the Phase 3 lift.
        const { matchesProfile } = await import('@decwebag/identity-resolver');
        const post = {
...
// keep the two in sync (or lift this trio into a tiny `@decwebag/cell-grid`
