...
import { startRelay }            from '@decwebag/relay';
...
//
// `ws` is only present under @decwebag/relay/node_modules/ws (transitive
// dep), so we resolve it through the relay's nested install too.
const __dirname  = dirname(fileURLToPath(import.meta.url));
const RELAY_NM   = join(__dirname, '../../../node_modules/@decwebag/relay');
...
describe('protocol — multi-recipient + relay restart (durable SQLite queue)', () => {
  ...
  beforeEach(async () => {
    tmpDir  = await mkdtemp(join(tmpdir(), 'decwebag-mrq-'));
