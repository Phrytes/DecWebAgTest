...
import { VaultMemory } from '@decwebag/vault';

import {
  createPseudoPod,
  createMemoryBackend,
}                                          from '@decwebag/pseudo-pod';
import { createPodRouting }                from '@decwebag/pod-routing';
import { createNotifyEnvelope }            from '@decwebag/notify-envelope';
import { validateCanonical }               from '@decwebag/item-types';
import { ItemStore, treeOf }               from '@decwebag/item-store';
import {
  createAgentRegistry,
  makeActorResolver,
}                                          from '@decwebag/agent-registry';
...
describe('substrates-v2 substrate-pipeline — end-to-end composition', () => {
  ...
  it('identity-resolver consumes agent-registry as a MemberMap (52.11 path)', async () => {
    ...
    const { createAgentRegistryMemberMap } = await import('@decwebag/identity-resolver');
