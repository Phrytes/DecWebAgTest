...
const MANIFEST_URI = 'https://alice.example/decwebag/manifest.ttl';
...
describe('identity/concurrent-manifest-write', () => {
  it('two simultaneous writers: LWW retry recovers; final manifest is consistent', async () => {
    ...
    expect(laptopRes.uri).toBe('https://alice.example/decwebag/devices/laptop.enc');
    expect(phoneRes.uri).toBe('https://alice.example/decwebag/devices/phone.enc');
  ...
  it('a single transient conflict on the resource (not the manifest) is also retried by the store', async () => {
    ...
    const authUri = `https://alice.example/decwebag/auth-log/${yyyy}-${mm}.enc`;
