...
> Peer of [`@decwebag/oidc-session-rn`](../oidc-session-rn/) (the React
> Native variant). Extracted from `@decwebag/core` 2026-05-11 as part of
...
```js
import { SolidVault } from '@decwebag/oidc-session';
...
`@decwebag/core`'s `VaultMemory` / `VaultNodeFs` / `VaultIndexedDB` /
...
`@decwebag/oidc-session-rn` and this package share the **consumer-facing
...
`@decwebag/core` continues to re-export `SolidVault` from its top-level
during the deprecation window. New consumers should import from
`@decwebag/oidc-session` directly:

```js
// old (still works, deprecated)
import { SolidVault } from '@decwebag/core';
...
- **52.15.4 / 52.15.5 shipped 2026-05-14** — web HTML picker +
  React Native `<IssuerPicker>` (`@decwebag/oidc-session-rn/picker`)
...
- [`@decwebag/oidc-session-rn`](../oidc-session-rn/) — RN peer.
- [`@decwebag/pod-client`](../pod-client/) — `SolidOidcAuth` wraps a
