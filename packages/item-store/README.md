...
```json
"dependencies": {
  "@decwebag/item-store": "file:../../packages/item-store"
}
```
...
```js
import { ItemStore, InMemoryBackend } from '@decwebag/item-store';
...
```js
import { ItemStore, InMemoryBackend, PermissionDeniedError } from '@decwebag/item-store';
...
- **PodBackend** — pod-backed via `@decwebag/pod-client` (Track A).
