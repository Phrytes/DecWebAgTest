...
@decwebag/core` (or via the file-link in this monorepo).
...
"dependencies": {
  ...
  "@decwebag/react-native": "file:../../packages/react-native",
  …
}
```
...
you cloned the SDK at `~/sdk/DecWebAgTest/`, you need three things:

1. **package.json** — point the file: links at the cloned SDK:

   ```json
   "dependencies": {
     "@decwebag/core":         "file:../../sdk/DecWebAgTest/packages/core",
     "@decwebag/react-native": "file:../../sdk/DecWebAgTest/packages/react-native"
   }
   ```
...
2. **metro.config.js** — Metro doesn't follow symlinks well, so
   ...
   const sdkRoot = path.resolve(__dirname, '../../sdk/DecWebAgTest');
   ...
   config.resolver = {
     ...
     extraNodeModules: {
       ...
       '@decwebag/core':         path.resolve(sdkRoot, 'packages/core'),
       '@decwebag/react-native': path.resolve(sdkRoot, 'packages/react-native'),
     },
   };
...
3. **Run the relay** from the SDK clone:
   `node ~/sdk/DecWebAgTest/packages/relay/src/server.js`.  Point your
...
If you want non-`@decwebag` clients (curl, browser fetch, IoT
