...
that Track A5 (`@decwebag/pod-client`) is complete, a real-end-to-end
...
- **S3 adapter** (`@decwebag/external-store-s3`) — likely first;
  most generic.
...
3. **Spec version not pinned in the card.**  Add
   `x-decwebag.a2aVersion: '<spec-version>'` so future bumps are
   detectable.
...
- `x-decwebag.a2aVersion` field on the card.
...
below) and the story for users wanting interop with non-`@decwebag`
agent frameworks.
...
surface for `@decwebag` agents.  Bespoke REST/JSON-RPC/GraphQL
