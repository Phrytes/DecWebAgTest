...
Goal: let non-`@decwebag` clients (web apps, IoT, cloud functions,
...
1. **Where does this live?**  New package
   `@decwebag/http-adapter` (depends on `@decwebag/core`)?  Or part
   of `@decwebag/relay` since both deal with HTTP/WS infra?
...
4. **Do we need a JS client library** (`@decwebag/http-client`)?
