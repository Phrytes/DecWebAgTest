...
- **`@decwebag/route`** (or `@decwebag/rpc` — naming TBD): the
  ...
  transport.
- **`@decwebag/http-adapter`** (Node + RN): HTTP gateway that
...
6. **Where to put the protocol-agnostic core**: own package vs.
   inside `@decwebag/core`.  Lean toward own package
   (`packages/route/` or `packages/rpc/`).
7. **JS client library** for HTTP callers (`@decwebag/route-client`)?
...
- **Negotiation-heavy flows** (sealed tunnel, key rotation)
  ...
  for the cases where a non-`@decwebag` client wants to invoke a
