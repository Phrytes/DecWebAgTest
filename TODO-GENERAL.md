...
**Status:** future feature.  Today's `@decwebag/relay` is a private-LAN
...
- `@decwebag/relay` versioning: clients and relays will need a clean
...
services, OR another `@decwebag` peer over the native mesh) talk to
...
#### `@decwebag/rest` — protocol-agnostic core
...
```js
import { attachRest, route } from '@decwebag/rest';
...
#### `@decwebag/http-adapter` — HTTP gateway (optional, Node + RN)
...
9. **Where to put `@decwebag/rest`** — its own package or part of
   `@decwebag/core`?  Lean: separate package (`packages/rest/`),
   peer-depends on core.  HTTP-adapter is a separate package on top.
10. **JS client library** (`@decwebag/rest-client`)?  For native
